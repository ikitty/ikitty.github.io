...
#===比较===
#语法：语句和左右的「」必须要有一个空格; 运算符号左右必须要有空格 ;
#[ 条件 ]不支持<> ，要使用 -lt -gt
[ 1 > 2 ] && echo ok || echo error #<>在标准的shell中被解释为重定向
[ 1 \> 2 ] && echo ok || echo error #use backslash to fix it 
[ 1 -gt 2 ] && echo ok || echo error
#ksh,zsh 等支持使用[[ test ]] 这种方式运行 <> ,据说原生shell不支持
[[ 1 > 2 ]] && echo ok || echo error
...
#keng 只能正确识别unix文件格式 「dos」格式的不行的哦
#for i in `cat ./u.txt` ;
...
#语法好贱啊，for两个括号
#awk -F'[ ]'
#-F的语法很奇怪，「」里面的内容的引号是可选的，而且外面还有一对单引号

#sed源文件中修改 -i 后面的空参数是必须的
#sed -i '' '1 d' file
